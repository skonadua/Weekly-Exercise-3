...
stephs_favs_DK <- tibble(
  place = c("Konditaget Lüders", "Amalienborg Slotsplads", "The Little Mermaid", "Kultorvet", "Tivoli", "The Black Diamond", "I Am Queen Mary", "Dragør Fort", "Amagerstrand", "Kongens Have & Rosenborg Slot", "Sankt Peders Bageri", "Ny Carlsberg Glyptotek", "Reffen"), #names
  lat = c(55.707, 55.68, 55.69, 55.6824, 55.673, 55.673, 55.686, 55.587 , 55.657,55.68, 55.679, 55.672, 55.693),
  long = c(12.59, 12.593, 12.599, 12.5735, 12.553, 12.583, 12.597, 12.677, 12.648, 12.577, 12.569, 12.571, 12.609),
  top3 = c("yes", "no", "no", "yes", "no", "no", "no", "no", "no", "no", "yes", "no", "no"))
...
covid19 %>% 
  ...
  geom_map(map = states_map, 
           aes(map_id = state.abbfill = cases)) +
  expand_limits(x = states_map$long, y = states_map$lat) +
  theme_map()

# DC_map <- get_stamenmap(
#   bbox = c(left = -77.31, bottom = 38.733, right = -76.73, top = 39.0539), 
#   maptype = "terrain",
#   zoom = 11)
# )
...
covid_cum <-
  ...
  mutate(per10 = (cases/est_pop_2018)*10000)
...
#summarize covid data and then join
...
pal_mpls2 <- colorFactor("viridis", domain = MplsStops$prop_suspicious)
...
MplsStops %>% 
  left_join(MplsProp, by = c("neighborhood")) %>% 
  leaflet() %>% 
  addTiles() %>% 
  addCircles(lng = ~long, 
             lat = ~lat,
             weight = 1,
             opacity = 1, 
             color = ~pal_mpls(prop_suspicious)) %>% 
  addLegend(position = c("topright"), 
            pal = pal_mpls, 
